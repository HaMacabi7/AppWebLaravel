...
class CompraManager extends CrudManager {
    ...
    addProductoToTable(item, cantidad = 1, precio_unitario = null, subtotal = null) {
        ...
        if (existingRow) {
            ...
            tr.innerHTML = `
                ...
                <td class="text-end">
                    <input type="number" name="detalles[${rowCount}][precio_unitario]" value="${precioConImpuesto.toFixed(2)}" min="1" class="form-control form-control-sm inputPrecioUnitario">
            ...
            tr.querySelector('.inputPrecioUnitario').addEventListener('change', () => this.calculateTotals());
    ...
    async showEditModal(id) {
        try {
            ...
            document.getElementById('proveedor_razon_social').value = response.proveedor?.razon_social || '';
    ...
    updateDetailsTable(detalles = []) {
        ...
        detalles.forEach(detalle => {
            this.addProductoToTable(detalle.producto, +detalle.cantidad, +detalle.precio_unitario, +detalle
                .subtotal);
        });
    }
